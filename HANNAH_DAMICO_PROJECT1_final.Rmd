...
```{r}
#Create the Table
Temp_tab <- frame2 %>% select(MRN, fever_range, temp_rounded) %>% filter(temp_rounded > 100) %>% group_by(fever_range) %>% arrange(desc(temp_rounded)) %>% rename("Fever Range" = "fever_range", "Temperature" = "temp_rounded")
...
## Race
```{r}
race_count <- frame1 %>% group_by(race) %>% summarise(Count = n()) %>% rename("Race" = "race")


race_prop <- 100*round(prop.table(table(frame1$race)),3)
...
```{r}
ethnic_count <- frame1 %>% group_by(ethnicity) %>% summarise(Count = n()) %>% rename("Ethnicity" = "ethnicity")


ethnic_prop <- 100*round(prop.table(table(frame1$ethnic)),3)
...
fin_class_count <- frame1 %>% group_by(financialclass) %>% summarise(Count = n()) %>% rename("Financial Class" = "financialclass")


fin_class_prop <- 100*round(prop.table(table(frame1$financialclass)),3)
...
## Hypertension
```{r}
hypertension_count <- frame1 %>% group_by(hypertension) %>% summarise(Count = n()) %>% rename("Hypertension" = "hypertension")


hypertension_prop <- 100*round(prop.table(table(frame1$hypertension)),3)
...
```{r}
CHF_count <- frame1 %>% group_by(CHF) %>% summarise(Count = n()) %>% rename("CHF" = "CHF")


CHF_prop <- 100*round(prop.table(table(frame1$CHF)),3)
...
## Diabetes
```{r}
diabetes_count <- frame1 %>% group_by(diabetes) %>% summarise(Count = n()) %>% rename("Diabetes" = "diabetes")


diabetes_prop <- 100*round(prop.table(table(frame1$diabetes)),3)
...
Race <- levels(frame1$race)
Ethnicity <- levels(frame1$ethnicity)
`Financial Class` <- levels(frame1$financialclass)
Hypertension <- levels(frame1$hypertension)
CHF <- levels(frame1$CHF)
Diabetes <- levels(frame1$diabetes)
